...
analyticsQueue.process("calculate-user-activity", 3, async (job) => {
  ...
  const activity = {
    ...
    submissions: {
      ...
      totalViews: submissions.reduce(
        (sum, s) => sum + (s.analytics?.views || 0),
        0
      ),
    ...
    reviews: {
      ...
      totalHelpful: reviews.reduce(
        (sum, r) => sum + (r.interactions?.helpful?.length || 0),
        0
      ),
    },
    comments: {
      ...
      totalLikes: comments.reduce(
        (sum, c) => sum + (c.reactions?.likes?.length || 0),
        0
      ),
    },
    calculatedAt: new Date(),
...
export const queueUserActivityCalculation = async (userId, period = 30) => {
  return await analyticsQueue.add("calculate-user-activity", {
    userId,
    period,
  });
};
